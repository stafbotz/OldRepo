	     ...
	     if (!isCmd && isGroup && !fromMe) console.log(`{\n`, color(` from: "${sender.split('@')[0]} - ${groupName}"\n  time: "${hour_now}"\n  command: "${command}"`,'yellow'), color(`\n}`,`white`))
             ...
             switch (command) {
                 ...
                 case 'kick' :
                     ...
                     let users = msg.message.extendedTextMessage.contextInfo ? msg.message.extendedTextMessage.contextInfo.mentionedJid[0] : msg.message.extendedTextMessage.contextInfo.quotedMessage ? msg.message.extendedTextMessage.contextInfo.participant.split(":")[0] || msg.message.extendedTextMessage.contextInfo.participant : q.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
